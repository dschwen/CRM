...
class SystemService {
    ...
    function restoreDatabaseFromBackup()
    {
        ...
        unlink($restoreResult->backupRoot);
        ...
        if ($restoreResult->type == "gz")
        {
            if ($restoreResult->type2 == "tar")
            {
                ...
                rename($restoreResult->backupRoot/Images, $restoreResult->root/Images);
            }
            else if ($restoreResult->type2 == "sql")
            {
                rename($file['tmp_name'], $restoreResult->backupRoot."/".$file['name']);
    ...
    function download($filename)
    {
        ...
        if (file_exists($path))
        {
            ...
            fclose ($fd);
            unlink(dirname(dirname(__FILE__))."/tmp_attach/ChurchCRMBackups");
        }
    }
